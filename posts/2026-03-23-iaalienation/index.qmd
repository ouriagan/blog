...
pts <- data.frame(
  ...
  y     = c(9, 9, 1, 1),
  label = c("S", "a'", "a", "A"),
  fill_col  = c("black", "black", "white", "white"),
  text_col  = c("white", "white", "black", "black")
)
...
p <- ggplot() +
  ...
  geom_text(
    data = pts,
    aes(x, y, label = label, colour = text_col),
    size = 11,
    family = "sans"
  ) +
...
![Schéma L (Lacan, 1978, p. 284)](schema_lacan.svg){#fig-schemalacan}
