...
les domaines et facettes en français est proposée ici. 
...
L'objectif de notre étude est de nous baser sur la traduction déjà existante de @gravel_french_nodate disponible sur le site de l'IPIP NEO. Cette traduction a suivi les trois première étapes de la méthode de @vallerand_vers_1989. Cette procédure incluait une première traduction en français, suivi d'une back-translation en anglais, qui devait correspondre avec la version originale. De ce fait, certains items ont été modifié jusqu'à trois fois pour correspondre au mieux et le pronom "Je" a dû être ajouté dans la version française. Ainsi, ce questionnaire a fait l'objet d'une traduction, mais jamais d'une validation à proprement parler.
...
Avant d'aller plus loin, il est important d'apporter la précision suivante : @gravel_french_nodate n'a a priori pas réalisé le travail de traduction des domaines et des facettes du questionnaire. Ainsi, les traductions proposées ci-après seront celles utilisées dans la suite de ce travail (tableau \@ref(tab:labels)).

```{r labels,  echo=FALSE}
kbl(ipineo_labels.df, caption = "Proposition de traduction française des domaines et facettes de l'IPIP NEO 300 items", booktabs = T) 
```
...
"investis" ont été codées "I". De cette manière, il a alors été possible de comptabiliser les différents thèmes afin de dégager la traduction donnée la plus fréquemment par les participants, celle qui se rapproche le plus de la meilleure traduction. Ainsi, pour cet item, le tableau \@ref(tab:workhard) a pu être dressé.


```{r workhard, echo=FALSE}
kbl(workhard.df, caption="Fréquences et proportions de traductions pour l'item « Work hard » par 20 étudiants en traduction de l'UMONS", booktabs = T)  
```
...
Dans le tableau \@ref(tab:traductions), voici les nouvelles traductions retenues, avec la proportion de participants ayant fourni cette traduction (ce tableau n'inclut pas les items inchangés).

```{r traductions, echo=FALSE}
kbl(traductions.df, caption="Traductions des items retenues dans le questionnaire", booktabs = T)  
```

Il est donc à présent possible de fournir une traduction française Européenne à l'IPIP NEO, en réutilisant la traduction québécoise validée et en replaçant tous les items ci-dessus par les nouvelles traductions fournies par notre panel de vingt étudiants traducteurs. La version française européenne que nous proposons est dans le tableau \@ref(tab:ipipneo).

# IPIP NEO 300 - Version française

```{r ipipneo, echo=FALSE}
kbl(ipipneo_300fr.df, caption="IPIP NEO 300 - Version française", longtable = T, booktabs = T) %>%
