...
Le score O+A+C du NEO PI moyen pour notre échantillon est égal à `r printnum(mean(alliance.df$alliance1))` (ET = `r printnum(sd(alliance.df$alliance1))`, min. = `r printnum(min(alliance.df$alliance1))`, max. = `r printnum(max(alliance.df$alliance1))`). 
La figure \@ref(fig:taballiancepsy) indique la fréquence des scores de qualité d'alliance thérapeutique évaluée par les psychologues pour chaque sujet.

```{r taballiancepsy, fig.cap="Qualité d'alliance thérapeutique", echo = FALSE}
...
```{r allianceoac, fig.cap="Qualité de l'alliance thérapeutique", echo = FALSE, preview=TRUE}
plotmeans(alliance.df$alliance1 ~ alliance.df$alliancepsy, xlab="Qualité de l'alliance thérapeutique", ylab="O+A+C")
```

La figure \@ref(fig:allianceoac) illustre cette absence de corrélation : les moyennes obtenues pour chaque score d'alliance thérapeutique évaluée par le / la psychologue ne diffèrent pas les unes des autres de manière significative.
...
La figure \@ref(fig:therapieoac)  permet de constater visuellement que les scores pour O+A+C sont globalement identiques pour les deux groupes.

```{r therapieoac, fig.cap="Score de O+A+C selon l'indication de prise en charge thérapeutique", echo = FALSE}
...
```{r Nalliancepsy, fig.cap="Score de N selon la qualité d'alliance thérapeutique", echo = FALSE}
plotmeans(N ~ alliancepsy, xlab="Qualité de l'alliance thérapeutique")
```

La figure \@ref(fig:Nalliancepsy) semble suggérer que les scores de névrosisme seraient plus élevés lorsque les psychologues rapportent une alliance thérapeutique bonne ou très bonne alors que les scores seraient inférieurs pour les alliances thérapeutiques inférieures. Pour vérifier cette impression, nous créons deux groupes : un premier groupe de détenus pour lesquelles l'alliance a été cotée 1 ou 2 (mauvaise alliance) et un second groupe pour lesquels l'alliance a été cotée 3 ou 4 (bonne alliance). Quels sont les scores de névrosisme pour ces deux groupes ?

```{r Nalliancepsy2, fig.cap="Score de N selon la qualité d'alliance thérapeutique", echo = FALSE}
...
La figure \@ref(fig:Nalliancepsy2) montre que si nous comparons les scores de névrosisme pour ces deux groupes de sujets, nous constatons qu'ils sont significativement différents (*t* = `r ttestnalliancebinaireparameter`, *p* = `r printp(ttestnalliancebinairepvalue)`).
...
kbl(apa_persotherapiereg$table, booktabs = T, caption = "Table de régression prédisant l'alliance thérapeutique")
```

Le modèle de régression présenté dans le tableau \@ref(tab:apapersotherapiereg) permet de constater que seule la variable N permet de prédire le score de la variable *therapie* de manière significative : `r apa_persotherapiereg$full_result$modelfit$r2`.  
...
La figure \@ref(fig:therapien) représente le lien entre la variable N et la variable therapie. Ainsi peut-on constater que lorsque le score en N augmente, la probabilité que le / la psychologue encourage une prise en charge thérapeutique augmente.

```{r therapien, fig.cap="Lien entre la variable N et la variable therapie", echo = FALSE}
...
Le tableau \@ref(tab:tableNcor) montre que deux factettes du névrosisme seraient significativement corrélées avec la qualité de l'alliance thérapeutique : N6 (Vulnérabilité au stress) et N3 (Dépression).
...
kbl(apa_lmN6N3alliancereg$table, booktabs = T, caption = "Table de régression prédisant l'alliance thérapeutique")
```
...
kbl(apa_lmN6alliancereg$table, booktabs = T, caption = "Table de régression prédisant l'alliance thérapeutique")
```

Ce modèle présenté dans le tableau \@ref(tab:apalmN6alliancereg) nous permet de constater que la variable N6 permet de prédire une partie de la variance de la variable de qualité d'alliance thérapeutique : `r apa_lmN6alliancereg$full_result$modelfit$r2`.  

```{r alliancenreg, fig.cap="Lien entre la variable N6 et l'alliance thérapeutique", echo = FALSE}
plot(N6, alliance.df$alliancepsy, ylab="Alliance")
points(N6,N6alliancereg_ypredit, col="red")
```

La figure \@ref(fig:alliancenreg) indique que plus le score de N6 augmente, au plus l'alliance est estimée meilleure par les psychologues.
...
Le tableau \@ref(tab:tableNbiscor) indique quatre variables seraient significativement corrélées avec l'indication de prise en charge : N3 (Dépression), N6 (Vulnérabilité au stress), N4 (Timidité sociale) et N1 (Anxiété). Afin d'étudier ces liens de manière plus approfondie, nous construisons un modèle de régression logistique qui tente de prédire l'indication thérapeutique avec quatre variables indépendantes : N3, N6, N4 et N1.
...
kbl(apa_lmN3N4N6N1therapiereg$table, booktabs = T, caption = "Table de régression prédisant l'indication thérapeutique")
```

L'introduction des quatre variables dans le modèle prédictif semble toutefois l'affaiblir comme l'indique le tableau \@ref(tab:apalmN3N4N6N1therapiereg). Après comparaison de différents modèles pour lesquels une seule de ces variables est prise comme variable indépendante, il apparaît que c'est le modèle qui implique N3 qui est le plus significatif, les autres variables n'apportant pas d'information supplémentaire.
...
kbl(apa_lmN3therapiereg$table, booktabs = T, caption = "Table de régression prédisant l'indication thérapeutique")
```

Le tableau \@ref(tab:apalmN3therapiereg) indique que la variable N3 présente une meilleure significativité lorsqu'elle est prise seule dans le modèle prédictif de la tendance des psychologues à poser une indication thérapeutique. 
