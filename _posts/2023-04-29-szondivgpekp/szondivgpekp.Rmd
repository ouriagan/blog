---
title: "Comparaison psychométrique des tensions entre l'avant et l'arrière-plan du test de Szondi"
description: |
  Le test de Szondi est un test projectif qui se compose de deux plans : l'avant-plan (VGP) et l'arrière-plan (EKP). Sur un échantillon de 451 détenus, nous étudions les tensions obtenues aux deux plans et les corrélations entre elles. Les choix opérés à l'avant-plan et à l'arrière-plan sont positivement corrélés. Les corrélations les plus élevées concernent les facteurs s et d. Les corrélations les plus faibles concernent les facteur e, k et m.
author:
  - first_name: "Benjamin"
    last_name: "Thiry"
...
Les deux photographies restantes sont considérées comme sympathiques, par défaut.
Les douze premiers choix sympathiques et les douze premiers choix antipathiques (soit 24 choix) constituent l'avant-plan (VGP - VorderGrundProfil) alors que les 24 autres choix constituent l'arrière-plan (EKP - Experimentelle  Komplementär Profil).

Au test de Szondi, la *tension* résulte des choix positifs ou négatifs des photographies par le participant. S'il choisit de nombreuses photographies comme étant *sympathiques*, la tension est qualifié de *positive* et s'il choisit de nombreuses photographies comme étant *antipathiques*, la tension est qualifiée de *négative*. 
Ces tensions peuvent différer à l'avant-plan et à l'arrière-plan.
...
La figure \@ref(fig:tensionsvgp) montre les scores de tension pour chaque facteur à l'avant-plan.

```{r tensionsvgp, fig.cap="Tensions au VGP du test de Szondi", echo = FALSE}
...
La figure \@ref(fig:tensionekp) montre les scores de tension pour chaque facteur à l'arrière-plan.

```{r tensionekp, fig.cap="Tensions à l'EKP du test de Szondi", echo = FALSE}
...
La figure \@ref(fig:vgpekpcor) montre les corrélations (Rho de Spearman) entre toutes les tensions à l'avant-plan et celles obtenues à l'arrière-plan.

```{r vgpekpcor, fig.cap="Corrélations entre les tensions au VGP et à l'EKP au test de Szondi", echo = FALSE}
...
A l'heure actuelle, il nous est impossible de dégager les pistes interprétatives d'un point de vue psychologique sur base de nos résultats. Ils permettent toutefois de mieux comprendre les liens entre les deux plans du test de Szondi.
