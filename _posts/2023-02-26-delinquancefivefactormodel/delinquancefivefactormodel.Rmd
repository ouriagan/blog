...
1. Avez-vous déjà eu l’un des problèmes suivants ? 
  ...
  - Rapport sexuel que vous regrettez le lendemain
...
La figure \@ref(fig:crimino) présente les distributions des trois échelles criminologiques que nous avons créées.

```{r crimino, fig.cap="Distributions des trois échelles criminologiques", echo=FALSE}
vioplot(crimino.df)
```
...
kbl(problemes, booktabs = T, caption = "Fréquences des problèmes auto-révélés par les 112 adolescents")
```


Le tableau \@ref(tab:problemes) reprend les fréquences des problèmes rencontrés par les adolescents de notre échantillon. Les trois problèmes les plus fréquents sont : avec les parents (71% des adolescents), des brouilles ou des disputes (66%) et des mauvais résultats à l’école (65%).

```{r delinq, echo=FALSE}
kbl(delinq, booktabs = T, caption = "Fréquences des faits de délinquance auto-révélés (au moins 1 fois) par les 111 adolescents")
```

Le tableau \@ref(tab:delinq)^[nous avons exclu le participant 20 car il avait indiqué les fréquences maximales pour toutes les questions relatives à la délinquance, ce que nous avons considéré suspect] reprend les fréquences des faits de délinquance prétendument commis par les adolescents de notre échantillon. Les trois faits de délinquance les plus fréquents sont : le vol (27%), le vandalisme scolaire (14%) et le vandalisme public (14%).

```{r victimo, echo=FALSE}
kbl(victimo, booktabs = T, caption = "Fréquences des faits de victimisation auto-révélés (au moins 1 fois) par les 112 adolescents")
```


Le tableau \@ref(tab:victimo) reprend les fréquences des faits de victimisation prétendument subis par les adolescents de notre échantillon. Les trois faits de victimisation les plus fréquents sont : les objets personnels abîmés (15%), les vols avec effraction (11%) et un vol de plus de 15 Euros (10%).
...
La figure \@ref(fig:scatterplot1) représente le lien entre le score d'antisocialité du NEO PI-R et le nombre de problèmes rapportés par les participants.

```{r scatterplot1, fig.cap="Lien entre le score d'antisocialité du NEO PI-R et le score auto-rapporté de problèmes", echo=FALSE}
...
```{r lm01, echo = FALSE}
kbl(apa_lm01$table, booktabs = T, caption = "Table de régression prédisant les problèmes de délinquance")
