...
1. Des personnes **tout-venant** qui avaient répondu à une annonce dans le cadre d'une recherche précédente [@thiry2009ecriture] : *n* = `r nofC` (`r print(nofCpercentage)`%) ;
2. Des personnes **condamnées** et emprisonnées : *n* = `r nofcondamnes` (`r print(nofcondamnespercentage)`%) ;
3. Des personnées emprisonnées mais reconnues **irresponsables pour cause de trouble mental** : *n* = `r nofinternes` (`r print(nofinternespercentage)`%) ;
4. Des **adolescents** qui ont passé le NEO PI-R en milieu scolaire : *n* = `r nofado` (`r print(nofadopercentage)`%) ; 
5. Des **étudiants** en Master de psychologie légale à l'Université de Mons : *n* = `r nofetudiant` (`r print(nofetudiantpercentage)`%) ;
6. Des **patients** en thérapie : *n* = `r nofpatient` (`r print(nofpatientpercentage)`%).

# Résultats

La Figure \@ref(fig:alliance1) représente la distribution de l'indice que nous avons appelé *alliance1* (domaines qui facilitent l'alliance thérapeutique). 
...
La Figure \@ref(fig:alliance2) représente la distribution de l'indice que nous avons appelé *alliance2* (facettes qui facilitent l'alliance thérapeutique). 
...
La Figure \@ref(fig:alliance3) représente la distribution de l'indice que nous avons appelé *alliance3* (facettes qui feinent l'alliance thérapeutique). 
...
A priori, les deux premiers indices évaluent une même tendance avec des variables différentes (**domaines** versus **facettes** du modèle de personnalité en cinq facteurs). Le troisième indice est censé évaluer une tendance inverse. Qu'en est-il des corrélations entre ces trois indices ?
...
Ces résultats nous invitent à remettre en question la pertinence d'utiliser trois indices pour évaluer une même tendance. Il semble que l'indice *alliance1* apporte une information substantielle pour l'évaluer. Notons qu'elle repose sur les scores de trois domaines (A, O et C) et non sur des facettes sous-jacentes (ce qui est un résultat non attendu a priori). Dans un souci d'économie, il est dès lors envisageable d'utiliser **un seul indice** : *alliance1* (A+O+C).
