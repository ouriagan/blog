...
nofmenpercentage <- round(100*(length(which(sexe == "M"))/length(N)), digit = 2)

nofwomen <- length(which(sexe == "F")) 
nofwomenpercentage <- round(100*(length(which(sexe == "F"))/length(N)), digit = 2)
...
A l'occasion d'une étude précédente [@thiry2020assessing] ([disponible ici](../../posts/2022-07-31-assessing-the-therapeutic-alliance-with-the-five-factor-model/)), nous avons demandé à 38 cliniciens du domaine de la santé mentale d'identifier les **traits de personnalité** de leurs patients susceptibles de **faciliter** ou de **freiner** l'**alliance thérapeutique**. 
Sur base de leurs réponses, nous avons identifié trois indices qui se basent sur les variables du modèle de personnalité en cinq facteurs (tels que le NEO PI [@mccrae2016inventaires] ou l'IPIP NEO [@goldberg2006international]) :

1. alliance1 (facilite) = O + A + C
2. alliance2 (facilite) = A2 + O3 + C5 + O5
3. alliance3 (freine) = (32-A5) + (32-A2) + (32-A1)

Nous n'avions toutefois fourni aucune donnée psychométrique relative à ces trois indices expérimentaux. Dans cet article, nous proposons des normes permettant de situer un protocole donné de NEO PI R (ou 3) par rapport à un échantillon normatif. 
...
Nous disposons d'un échantillon vaste mais hétéroclite. Il se compose de `r ntotal` personnes dont `r nofmen` (`r nofmenpercentage`%) hommes et `r nofwomen` (`r nofwomenpercentage`%) femmes qui ont répondu à la version française du NEO PI R ou 3. 
L'âge moyen est égal à `r meanage` ans avec un écart-type égal à `r sdage`. L'âge varie de `r minage` à `r maxage` ans.

L'échantillon global se compose de six sous-échantillons :

1. Des personnes **tout-venant** qui avaient répondu à une annonce dans le cadre d'une recherche précédente [@thiry2009ecriture] : `r nofC` (`r printnum(nofCpercentage)`%) ;
2. Des personnes **condamnées** et emprisonnées : `r nofcondamnes` (`r printnum(nofcondamnespercentage)`%) ;
3. Des personnées emprisonnées mais reconnues **irresponsables pour cause de trouble mental** : `r nofinternes` (`r printnum(nofinternespercentage)`%) ;
4. Des **adolescents** qui ont passé le NEO PI-R en milieu scolaire : `r nofado` (`r printnum(nofadopercentage)`%) ; 
5. Des **étudiants** en Master de psychologie légale à l'Université de Mons : `r nofetudiant` (`r printnum(nofetudiantpercentage)`%) ;
6. Des **patients** en thérapie : `r nofpatient` (`r printnum(nofpatientpercentage)`%).

# Résultats

La Figure \@ref(fig:age) représente la distribution des âges (connus) pour notre échantillon total. Notons qu'il est assez jeune (*M* = `r printnum(meanage)`, *ET* = `r printnum(sdage)`).

```{r age, fig.cap="Ages de l'échantillon total", echo = FALSE}
hist(age, nclass=15, col="cornflowerblue", border="white", xlim=c(00,90), main="", xlab="Âge (années)", ylab="Fréquence")
text(60, 100, paste("N =", sum(complete.cases(age))), cex=1.2)
```


La Figure \@ref(fig:alliance1) représente la distribution de l'indice que nous avons appelé *alliance1*. 
...
La Figure \@ref(fig:alliance2) représente la distribution de l'indice que nous avons appelé *alliance2*. 

```{r alliance2, fig.cap="Distribution d'alliance2", echo = FALSE}
...
La Figure \@ref(fig:alliance3) représente la distribution de l'indice que nous avons appelé *alliance3*. 
...
apa_table(
  table1
  , caption = "Statistiques descriptives des trois variables d'alliance thérapeutique"
)
```

Le Tableau \@ref(tab:table1) présente les statistiques descriptives des trois variables d'alliance thérapeutique. Elles permettent notamment de calculer des scores *t* à l'instar des autres variables du NEO PI.
Une question se pose : nos différents groupes présentent-ils des scores différents pour chacune de nos variables ?

## Moyennes d'alliance 1 par groupe

La Figure \@ref(fig:alliance1groupe) présente les moyennes et les intervalles de confiance pour chaque groupe pour la variable *alliance1*.

```{r alliance1groupe, fig.cap="Moyennes pour Alliance1 par groupe", echo = FALSE}
...
apa_table(
  apa_lm1$table
  , caption = "Table de régression pour alliance 1 par groupes"
)
```

Nous constatons dans le Tableau \@ref(tab:lm1) que les moyennes pour les groupes *adolescent* et *interné* sont significativement plus basses que les autres. Par contre toutes les autres moyennes sont estimées identiques à celle du groupe *contrôle*.

## Moyennes d'alliance 2 par groupe

La Figure \@ref(fig:alliance2groupe) présente les moyennes et les intervalles de confiance pour chaque groupe pour la variable *alliance2*.

```{r alliance2groupe, fig.cap="Moyennes pour Alliance2 par groupe", echo = FALSE}
...
apa_table(
  apa_lm2$table
  , caption = "Table de régression pour alliance 2 par groupes"
)
```

Nous constatons dans le Tableau \@ref(tab:lm2) que la moyenne pour le groupe *adolescent* est significativement plus basse que les autres. Par contre toutes les autres moyennes sont estimées identiques à celle du groupe *contrôle* au seuil *p* <.001.

## Moyennes d'alliance 3 par groupe

La Figure \@ref(fig:alliance3groupe) présente les moyennes et les intervalles de confiance pour chaque groupe pour la variable *alliance3*.

```{r alliance3groupe, fig.cap="Moyennes pour Alliance3 par groupe", echo = FALSE}
...
Nous constatons dans le Tableau \@ref(tab:lm3) que la moyenne pour le groupe *adolescent* est significativement plus élevée que les autres. Par contre toutes les autres moyennes sont estimées identiques à celle du groupe *contrôle*.

## Les variables d'alliance thérapeutique diffèrent-elles d'un groupe à l'autre

D'une manière générale, nous **ne** pouvons **pas** affirmer que nos groupes ont un effet sur les scores aux trois indices d'alliance thérapeutique, à part le groupe *adolescent*. Par conséquent, les normes proposées au Tableau \@ref(tab:table1) peuvent dès lors s'appliquer à tous les participants. Notons toutefois que l'**âge** des participants est toutefois susceptible d'être lié à ces scores. Nous devrons dès lors nous intéresser à cette variable afin d'en apprendre davantage.


## Liens entre les trois indices

A priori, les deux premiers indices évaluent une même tendance avec des variables différentes (**domaines** versus **facettes** du modèle de personnalité en cinq facteurs). Le troisième indices est censé évaluer une tendance inverse. Qu'en est-il des corrélations entre ces trois indices ?
La Figure \@ref(fig:correlations) représente les corrélations entre les trois indices.

```{r correlations, fig.cap="Corrélations entre les trois indices", echo = FALSE}
...
Nous constatons effectivement que les indices *alliance1* et *alliance2* sont fortement corrélés positivement, et négativement avec *alliance3*.
Ces résultats nous invitent à remettre en question la pertinence d'utiliser trois indices pour évaluer une même tendance. Il semble que l'indice *alliance1* apporte une information substancielle pour l'évaluer. Notons qu'elle repose sur les scores de trois domaines (A, O et C) et non sur des facettes sous-jacentes (ce qui est un résultat non attendu a priori). Dans un souci d'économie, il est dès lors envisageable d'utiliser **un seul indice** : *alliance1* (A+O+C).
...
```{r alliage, fig.cap="Effet de l'âge et du sexe sur alliance1", echo = FALSE}
scatterplot(alliance1~age, data=neototal)
```

La Figure \@ref(fig:alliage) semble indiquer une tendance des deux variables à augmenter l'une avec l'autre. Notons l'infléchissement de la courbe loess pour les **âges inférieurs à vingt ans** (ce qui correspond à notre groupe d'adolescents). Il semble que les scores d'*alliance1* soient particulièrement bas avant cet âge.
...
La Figure \@ref(fig:alliance1sexe) semble indiquer que les femmes obtiennent des scores supérieurs que les hommes à la variable *alliance1*.
...
apa_table(
  apa_lm4$table
  , caption = "Table de régression pour alliance 1 par age et sexe"
)
```

Les deux variables semblent avoir un effet signfificatif sur la variable *alliance1* : `r apa_lm4$full_result$modelfit$r2`

# Discussion

Que nous apprennent ces résultats ? D'abord que les trois variables présentent permettent de discriminer les sujets de manière assez large. Ensuite que les personnes incarcérées, les patients et les étudiantes obtiennent des scores globalement identiques aux trois indices. Les personnes dont l'âge est inférieur à vingt ans présentent quant à eux des scores à alliance1 significativement inférieurs aux autres. Les hommes également présentent des scores à alliance1 significativement plus bas que les femmes.
L'usage des trois indices est susceptible d'approcher le phénomène d'alliance thérapeutique de manière nuancée en termes dynamiques : ce qui facilite versus ce qui freine l'alliance thérapeutique. Toutefois, les trois indices partagent des liens étroits les uns avec les autres. Ils semblent dès lors apporter une informations fortement redondante (et donc inutile). Dans un souci de simplicité et d'économie, il semble que la variable alliance1 (O+A+C) puisse suffir pour évaluer le phénomène qui nous intéresse. Contrairement à nos attentes initiales, les variables estimées pertinentes pour prédire une bonne alliance thérapeutiques par les cliniciens ne reposent pas sur des facettes précises de la personnalité mais plutôt sur des configurations plus globales.
Ce profil O^+^ A^+^ C^+^ appelle dès lors à commentaires. D'abord, il ne s'avère pas uniquement pertinent en regard d'une prise en charge psychothérapeutique. Il renvoie à un rapport particulier que les individus entretiennent avec le monde extérieur, les autres personnes et les stratégies d'actions. Les *styles graph* du manuel du NEO PI 3 [@mccrae2016inventaires] permettent de dégager trois styles relatifs à cette configuration :
